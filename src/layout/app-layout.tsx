...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'financial-analyst', 'sales-representative', 'customer-support'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<FirmInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/firms` : null),
    () => getFirms({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Firm',
      path: isTenantUser ? `/firms/view/${data?.[0]?.id}` : '/firms',
      entity: 'firm',
      service: AccessServiceEnum.PROJECT,
      icon: FiBriefcase,
    },
    { name: 'Customer', path: '/customers', entity: 'customer', service: AccessServiceEnum.PROJECT, icon: FiUsers },
    { name: 'Query', path: '/queries', entity: 'query', service: AccessServiceEnum.PROJECT, icon: FiHelpCircle },
    { name: 'Report', path: '/reports', entity: 'report', service: AccessServiceEnum.PROJECT, icon: FiFileText },
